...
function buildUserPrompt(role, region, industry, lifecycle, name, company, context, humor, warmth, directness) {
  ...
  prompt += `\nVälj den approach som datan visar fungerar bäst för denna kombination av roll × bransch × region. Ge rekommendation för optimal dag/tid för varje steg baserat på svarsdata. INGA talstreck (— –) i texten. Svara BARA med JSON.`;
    ...
    <div style={{ background: P.bg, minHeight: "100vh", color: P.tx, fontFamily: "system-ui,-apple-system,sans-serif" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "20px 24px 48px" }}>
        ...
        {result && (
          <div>
            {/* Strategy summary */}
            <div style={{ background: P.gS, border: `1px solid ${P.g}30`, borderRadius: 12, padding: "16px 20px", marginBottom: 12 }}>
            ...
            {result.timing_recommendation && (
              <div style={{ background: P.amS, border: `1px solid ${P.am}30`, borderRadius: 12, padding: "16px 20px", marginBottom: 20 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: P.am, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 4 }}>Rekommenderad utskicksplan</div>
                <div style={{ fontSize: 13, color: P.tx, lineHeight: 1.6 }}>{result.timing_recommendation}</div>
              </div>
            )}
